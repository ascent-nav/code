...
class BLIP_SG_Nav_Policy(BasePolicy):
    def __init__(
        self,
        # config,
        # full_config,
        ...
        **kwargs: Any,
        ...
        self.visualization_dir = f'debug/20250212/v2' # {self.experiment_name}/
    ...
    def act(
        ...
        **kwargs,
        ...
        if self.total_steps <= 40 and not self.found_goal: #   # <= 22
            ...
            self.detect_objects(observations_numpy) # 得到goal或者long-term goal
    ...
    def _plan(self, traversible, goal_map, agent_pose, start, start_o, goal_found):
        ...
        if stop:
            ...
            goal_gps = [0.,0.]
            goal_gps[0] = (self.map_size_cm / (2 * self.resolution) - stg[0]) * self.resolution / 100 # stg[0] * self.resolution / 100 / self.map_size_cm * 100.0 * 2.0
            goal_gps[1] = (self.map_size_cm / (2 * self.resolution) - stg[1]) * self.resolution / 100 # stg[1] * self.resolution / 100 / self.map_size_cm * 100.0 * 2.0
            # elif self.found_goal:
            #     goal_gps = self.goal_gps
            # elif self.found_long_goal:
            #     goal_gps = self.long_goal_temp_gps
            robot_xy = np.array([self.observations['gps'][0][0].item(), - self.observations['gps'][0][1].item()])
            heading = self.observations['compass'][0].item()
            rho, theta = rho_theta(robot_xy, heading, goal_gps)
            rho_theta_tensor = torch.tensor([[rho, theta]], device="cuda", dtype=torch.float32)
            # direction_vector = goal_gps - np.array([self.observations['gps'][0][0].item(),self.observations['gps'][0][1].item()])
            # rho = np.sqrt(direction_vector[0]**2 + direction_vector[1]**2)
            # phi_world = np.arctan2(direction_vector[1], direction_vector[0])
            # agent_compass = self.observations['compass'][0]
            # phi = phi_world - agent_compass
            # rho_theta_tensor = torch.tensor([[rho, phi.item()]], device="cuda", dtype=torch.float32)
            obs_pointnav = {
                "depth": image_resize(
                    self.observations["depth"],
                    (224,224), # self._image_height[0], self._image_width[1]
                    channels_last=True,
                    interpolation_mode="area",
                ),
                "pointgoal_with_gps_compass": rho_theta_tensor,
            }
            action = self._pointnav_policy.act(obs_pointnav, self.masks, deterministic=True)
            action = action.item()
            (stg_y, stg_x) = stg
