...
class HabitatITMPolicy_Qwen_Gdino_IDEAL(HabitatMixin, ITMPolicyV2):
    def __init__(
        self,
        *args: Any,
        **kwargs: Any,
    ...
    def _update_value_map(self) -> None:
        ...
        for env in range(self._num_envs):
            # 提取当前环境的 RGB 图像
            rgb = self._observations_cache[env]["value_map_rgbd"][0][0]
            all_rgb.append(rgb)
    ...
    def act(
        ...
        **kwargs: Any,
        ...
        if "scene_id" in kwargs: 
            if self.semantic_dict["scene_id"] == kwargs["scene_id"] and self.semantic_dict["target_ids"] == self._target_object[0]:
                pass
            else:
                self.semantic_dict = self.extract_scene_with_target(kwargs["scene_id"], self._target_object[0])
...
# 示例优化后的函数
    def _get_best_frontier_with_llm(
        ...
        frontier_selection_count = defaultdict(int)
        ...
        close_frontiers = [
            ...
            for idx, (frontier, distance) in enumerate(zip(frontiers, distances)) 
            if distance <= 3.0
        ]
        ...
        if np.array_equal(self._last_frontier[env], best_frontier):
            ...
            self._frontier_stick_step[env] = 0
            self._last_frontier_distance[env] = 0
            if frontier_tuple in frontier_selection_count:
                self._force_frontier[env] = best_frontier.copy()

        # 第一次选中
        if frontier_tuple not in frontier_selection_count:
            frontier_selection_count[frontier_tuple] = 0
        # 第二次选中
        else:
            if not np.array_equal(self._last_frontier[env], best_frontier):
                frontier_selection_count[frontier_tuple] += 1
                if frontier_selection_count[frontier_tuple] >= 20:
                    self._obstacle_map[env]._disabled_frontiers.add(tuple(best_frontier))
                    print(f"Frontier {best_frontier} is disabled due to no movement.")
    ...
    def _update_object_map_with_stair_and_person(self, height: int, width: int): # -> Tuple[ ObjectDetections, List[np.ndarray] ]:
        ...
        for env in range(self._num_envs):
            ...
            # target_detections, coco_detections, 
            non_coco_detections = self._get_object_detections_with_stair_and_person(rgb, env) # get three detections #target_detections, person_detections, stair_detections 
            ...
            if self._observations_cache[env]["semantic"] is not None:
                semantic_data  = self._observations_cache[env]["semantic"]
                target_object_mask = np.zeros_like(semantic_data, dtype=bool)
                target_ids  = self.semantic_dict["target_ids"]
                # 创建掩码数组，初始值为 False
                target_object_mask = np.zeros_like(semantic_data, dtype=bool)
                ...
                # 如果 semantic_data 的形状是 (480, 640, 1)，可以去掉最后一个维度
