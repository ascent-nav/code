...
from vlfm.vlm.owl_vit_test import OwlV2Client_MF
...
class HabitatITMPolicy_Owl(HabitatMixin, ITMPolicyV2):
    def __init__(
        self,
        *args: Any,
        **kwargs: Any,
        ...
        self._object_detector = OwlV2Client_MF(port=int(os.environ.get("OwlV2_PORT", "13185")))
    ...
    def _get_best_frontier(
        ...
        # if not np.array_equal(np.zeros(2), self._force_frontier[env]):
        # if self._force_frontier[env] in frontiers: # and self._force_frontier[env] not in self._obstacle_map[env]._disabled_frontiers:
            best_frontier = self._force_frontier[env]
            frontier_tuple = tuple(best_frontier)
            # matches = [np.array_equal(frontier, self._force_frontier[env]) for frontier in frontiers]
    ...
    def _get_object_detections_with_stair_and_person(self, img: np.ndarray, env: int) -> ObjectDetections:
        ...
        # "tv_monitor", # "picture"
        all_caption = ["chair", "couch", "potted plant", "bed", "toilet", "tv"] + ["table", "wastebin", "washer", "picture"]+ ["stair", "person", " "]
        # all_caption = [[cls] for cls in target_classes] + [["stair"], ["person"], [" "]] 
        # 进行目标检测
        # if target_classes[0] == "potted plant":
        #     # target_classes = "potted plant"
        #     all_detections = self._object_detector.predict(image=img, caption=all_caption, score_thr=0.05)
        # else:
        # img_tensor = torch.from_numpy(img).float()
        # img_tensor = img_tensor.to(self.device)
        all_detections = self._object_detector.predict(image=img, caption=all_caption, box_threshold=0.2) # , score_thr=0.05) # default score_thr=0.5, 但楼梯好像有些时候0.5识别不到
        
        target_detections = deepcopy(all_detections)
        target_detections.filter_by_class(target_classes)  
        # if target_classes[0] == 'chair' or target_classes[0] == 'potted plant' or target_classes[0] == 'toilet':
        #     det_conf_threshold = 0.5
        # else:
        det_conf_threshold = 0.3 # 0.5
        target_detections.filter_by_conf(det_conf_threshold)

        person_detections = deepcopy(all_detections)
        person_detections.filter_by_class(["person"])  
        det_conf_threshold = 0.5 # self._coco_threshold # if target_in_coco else self._non_coco_threshold
        person_detections.filter_by_conf(det_conf_threshold)

        stair_detections = deepcopy(all_detections)
        stair_detections.filter_by_class(["stair"])  #(["stair", "step"]) # (["stair"])  
        # det_conf_threshold = 0.3 # self._coco_threshold # if target_in_coco else self._non_coco_threshold
        # stair_detections.filter_by_conf(det_conf_threshold)
        return target_detections, person_detections, stair_detections
    ...
    def _update_object_map_with_stair_and_person(self, height: int, width: int): # -> Tuple[ ObjectDetections, List[np.ndarray] ]:
        ...
        for env in range(self._num_envs):
            ...
            target_detections, person_detections, stair_detections = self._get_object_detections_with_stair_and_person(rgb, env) # get three detections
            ...
            for idx in range(len(target_detections.logits)):
                target_bbox_denorm = target_detections.boxes[idx].int() # * np.array([width, height, width, height])
                ...
                if self._target_object[env] == "bed":
                    ...
                    if np.any(seg_mask):
                        ...
                        # 计算 object_region
                        object_region = object_mask.astype(bool)
                        ...
                        if np.any(overlap):
                            ...
                            if overlap_percentage >= 0.34: # 1/3 有点危险
                                ...
                                if self._try_to_navigate[env] == True:
                                    self._double_check_goal[env] = True
                                    print("Double check success!!!")
                # elif self._target_object[env] == "chair":
                #     seg_mask = (self.red_semantic_pred_list[env] == CHAIR_CLASS_ID) # target_class_id
                #     if np.any(seg_mask):
                #         close_kernel = np.ones((7, 7), np.uint8)
                #         seg_mask_bool = cv2.morphologyEx(seg_mask.astype(np.uint8), cv2.MORPH_CLOSE, close_kernel)
                #         # 计算 object_region
            ...
            for idx in range(len(person_detections.logits)):
                person_bbox_denorm = person_detections.boxes[idx].int() # * np.array([width, height, width, height])
            ...
            for idx in range(len(stair_detections.logits)):
                stair_bbox_denorm = stair_detections.boxes[idx].int() # * np.array([width, height, width, height])
