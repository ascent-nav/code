...
class HabitatITMPolicy_Qwen_Gdino_IDEAL(HabitatMixin, ITMPolicyV2):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._object_detector = GroundingDINOClient_MF(port=int(os.environ.get("GROUNDING_DINO_PORT", "13184")))
        ...
        for env in range(self._num_envs):
            ...
            if not self._climb_stair_over[env] and self._climb_stair_flag[env] in (STAIR_UP, STAIR_DOWN):
                ...
                if self._reach_stair_centroid[env]:
                    if self.is_robot_in_stair_map_fast(env, robot_px, self._temp_stair_map[env])[0]:
                        pass
                        ...
                        # 重置状态
                        self._reset_stair_state(env)
                        self._end_of_climbing_stair(env)
                        print(f"Frontier {self._stair_frontier[env]} disabled")
                    else:
                        self._update_stair_endpoints(env,robot_px)
                        self._end_of_climbing_stair(env)
    ...
    def _end_of_climbing_stair(self, env: int) -> None:
    ...
    def _update_up_stair_endpoints(self, env: int, robot_px: np.ndarray) -> None:
        ...
        if not self._obstacle_map_list[env][self._cur_floor_index[env] + 1]._done_initializing:
            ...
            self._upstair_to_downstair(env)
        else:
            self._cur_floor_index[env] += 1
    ...
    def _update_down_stair_endpoints(self, env: int, robot_px: np.ndarray) -> None:
        ...
        if not self._obstacle_map_list[env][self._cur_floor_index[env] - 1]._done_initializing:
            ...
            self._downstair_to_upstair(env)
        else:
            self._cur_floor_index[env] -= 1
            self._update_current_floor_maps(env)


    def _upstair_to_downstair(self, env: int) -> None:
    ...
    def _downstair_to_upstair(self, env: int) -> None:
    ...
    def _get_object_detections_with_stair(self, imgs: List[np.ndarray], env_indices: List[int]) -> List[ObjectDetections]:
        # 为每个环境生成对应的描述
        non_coco_captions_with_stair = ["stair ." for _ in range(len(env_indices))]
    ...
    def _update_object_map_with_stair(self, height: int, width: int):
        ...
        if rgb_images:
            ...
            # 批量处理图片
            place365_input_imgs = torch.stack([self.place365_centre_crop(img) for img in pil_images]).to(self.device)
            ...
            for env in range(len(env_indices)):
                ...
                # for stair
                # non_coco_detections = self._get_object_detections_with_stair(rgb_images[env], env_idx)
                # 过滤检测结果
                self.non_coco_detection_list[env_idx].filter_by_class(["stair"])
                self.non_coco_detection_list[env_idx].filter_by_conf(self._non_coco_threshold) # ori 0.60
