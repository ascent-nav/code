...
def load_floor_probabilities(file_path):
    ...
    #     return None
floor_probabilities_df = load_floor_probabilities("/home/zeyingg/github/habitat-lab-vlfm/mp3d_combined_analysis_v2.xlsx")
...
class HabitatITMPolicy_Qwen_Gdino_DFine_MP3D_111(HabitatMixin, ITMPolicyV2):
    def __init__(
        self,
        *args: Any,
        **kwargs: Any,
        ...
        self._object_detector = GroundingDINOClient_MF(port=int(os.environ.get("GROUNDING_DINO_PORT", "11184")))
        self._coco_object_detector = DFineClient(port=int(os.environ.get("DFINE_PORT", "11186")))
        self._mobile_sam = MobileSAMClient(port=int(os.environ.get("SAM_PORT", "11183")))
        self._ram = RAMClient(port=int(os.environ.get("RAM_PORT", "11185")))
        
        self._use_vqa = kwargs["use_vqa"]
        # if self._use_vqa:
        #     self._vqa = BLIP2Client(port=int(os.environ.get("BLIP2_PORT", "11185")))
        ...
        self._coco_threshold = kwargs["coco_threshold"] # # 0.4 is the demo threshold in dfine. kwargs["coco_threshold"] # 0.8 in base_objectnav_policy，感觉0.7也不错？ 
        self._non_coco_threshold = kwargs["non_coco_threshold"] # # kwargs["non_coco_threshold"] # 0.4 in base_objectnav_policy
        ...
        self._object_map_list = [[ObjectPointCloudMap(erosion_size=self._object_map_erosion_size, size=1600,)] for _ in range(self._num_envs)]
        ...
        self._llm = Qwen2_5ITMClient(port=int(os.environ.get("QWEN2_5ITM_PORT", "11181")))
        self._itm = BLIP2ITMClient(port=int(os.environ.get("BLIP2ITM_PORT", "11182")))
        ...
        self._value_map_list = [ [ValueMap(
            ...
            obstacle_map=None,  size=1600, # self._obstacle_map if kwargs["sync_explored_areas"] else None,
        )]
        for _ in range(self._num_envs)]
    ...
    def _get_policy_info(self, detections: ObjectDetections,  env: int = 0) -> Dict[str, Any]: # seg_map_color:np.ndarray,
        ...
        # if self._person_masks[env].sum() > 0:
        ...
        #     annotated_depth = cv2.drawContours(annotated_depth, contours, -1, (255, 0, 0), 2)
        # if self._stair_masks[env].sum() > 0:
        ...
        masks_info = [
            ...
            (self._person_masks[env], (255, 105, 180)), # person: 粉色
        ...
        policy_info["object_map"] = cv2.cvtColor(self._object_map[env].visualize(), cv2.COLOR_BGR2RGB) # cv2.cvtColor( self._object_map[env].visualization,
                                                # cv2.COLOR_BGR2RGB) 
    ...
    def _update_obstacle_map(self, observations: "TensorDict") -> None: #  depth_for_stair_list: List[bool],
        for env in range(self._num_envs):
                    ...
                    if self._reach_stair_centroid[env] == True:
                        ...
                        if self.is_robot_in_stair_map_fast(env, robot_px, self._temp_stair_map[env])[0]: # (self._climb_stair_flag[env] == 1 and  # 
                            ...
                            if  self._climb_stair_flag[env] == 1:
                                ...
                                # remove from map list, also change the floor number
            ...
            if self._obstacle_map[env]._has_up_stair and self._cur_floor_index[env] + 1 >= len(self._object_map_list[env]):
                # 添加新的地图
                self._object_map_list[env].append(ObjectPointCloudMap(erosion_size=self._object_map_erosion_size,size=1600,)) 
            ...
            if self._obstacle_map[env]._has_down_stair and self._cur_floor_index[env] == 0:
                ...
                self._object_map_list[env].insert(0, ObjectPointCloudMap(erosion_size=self._object_map_erosion_size,size=1600,))
    ...
    def _explore(self, observations: Union[Dict[str, Tensor], "TensorDict"], env: int, masks: Tensor) -> Tensor:
        ...
        if np.array_equal(frontiers, np.zeros((1, 2))) or len(frontiers) == 0: # no frontier in this floor, check if there is stair
            ...
            if self._obstacle_map[env]._explored_up_stair == False:
                # 如果上楼的楼梯没爬过，上楼优先
            ...
            elif self._obstacle_map[env]._explored_down_stair == False:
                # 如果下楼的楼梯没爬过，下楼优先
                if self._obstacle_map[env]._has_down_stair:
                    # 有下楼的楼梯，检查更矮的楼层,直到楼下有没探索过的
                    for ith_floor in range(self._cur_floor_index[env] - 1, -1, -1):
                        if not self._obstacle_map_list[env][ith_floor]._this_floor_explored:
                            temp_flag = True
                            break
                    if temp_flag:
                        ...
                        pointnav_action = self._pointnav(observations, self._stair_frontier[env][0], stop=False, env=env)
                        return pointnav_action
                    elif self._obstacle_map[env]._has_up_stair:
                        # 没有下楼的楼梯，但有上楼的楼梯
                        for ith_floor in range(self._cur_floor_index[env] + 1, len(self._object_map_list[env])):
                            if not self._obstacle_map_list[env][ith_floor]._this_floor_explored:
                                temp_flag = True
            ...
            best_frontier, best_value = self._get_best_frontier_with_llm(observations, frontiers, env, use_multi_floor=True) # self._get_best_frontier(observations, frontiers, env)
        ...
        if self._pitch_angle[env] >= 0:
            ...
            if dis_to_potential_stair > 0.2:
                ...
                if pointnav_action.item() == 0:
                    ...
                    self._obstacle_map[env]._look_for_downstair_flag = False
                    pointnav_action = TorchActionIDs_plook.LOOK_UP.to(masks.device)
                    # remove from map list, also change the floor number, look for do not need!
                    # del self._object_map_list[env][self._cur_floor_index[env] - 1]
                    # del self._value_map_list[env][self._cur_floor_index[env] - 1]
                    # del self._obstacle_map_list[env][self._cur_floor_index[env] - 1]
                    # self.floor_num[env] -= 1
    ...
    def _get_best_frontier_with_llm(
        ...
        # 先检查是否有优先级最高的frontier best_value = sorted_values[best_frontier_idx]
        ...
        # 每层的第一步都禁用近邻导航
        # 近邻完之后再开启禁用
        if best_frontier is None and self._obstacle_map[env]._finish_first_explore == True:
            self._force_frontier[env] = np.zeros(2)
        ...
        # 如果没有，那么开始决策
            ...
            if len(sorted_pts) == 1:
                ...
                # 如果有多楼层判断，先做判断， # 然后再判断单层的
                elif self.floor_num[env] > 1 and self._num_steps[env] - self.multi_floor_ask_step[env] >= 60 and self._obstacle_map[env]._floor_num_steps >= 100 and use_multi_floor == True: # len(self._object_map_list[env][self._cur_floor_index[env]].this_floor_rooms) >= 5
                    # 加辅助条件，当前层发现大于等于5个房间,并且距离上一次问已经过了60  30步
                    self.multi_floor_ask_step[env] = self._num_steps[env] #.copy() 整型不需要
                    ...
                    if multi_floor_response == "-1":
                        ...
                        if temp_frontier_index > current_floor: # 上楼
                            return sorted_pts[0], -100
                        elif temp_frontier_index < current_floor: # 下楼
                            return sorted_pts[0], -200
                        else:
                            best_frontier_idx = self.llm_analyze_single_floor(env, target_object_category, frontier_index_list)  
                else: 
                    best_frontier_idx = self.llm_analyze_single_floor(env, target_object_category, frontier_index_list)  
        ...
        if np.array_equal(self._last_frontier[env], best_frontier):
            ...
            if self._frontier_stick_step[env] == 0:
                ...
                # 检查距离变化是否超过 1 米或者不是近邻搜索
                if np.abs(self._last_frontier_distance[env] - current_distance) > 0.3 and self._obstacle_map[env]._neighbor_search == False:
                    # 如果距离变化超过 1 米,重置步数和更新距离
                    self._frontier_stick_step[env] = 0
                    self._last_frontier_distance[env] = current_distance  # 更新为当前距离
                else:
                    # 如果步数达到 20 加没有明显的距离变化(< 0.3 米),或者近邻，那么禁用前沿
                    if self._frontier_stick_step[env] >= 20:
                        ...
                        self._frontier_stick_step[env] = 0 ## important
                    else:
                        # 如果没有达到 20 步,继续增加步数
                        self._frontier_stick_step[env] += 1
        else:
        ...
        # 第二次选中
            ...
            if np.array_equal(self._last_frontier[env], best_frontier):
                ...
                # 多次跳跃（非连续）选中，异常了，直接禁用
                if self._obstacle_map[env]._best_frontier_selection_count[frontier_tuple] >= 20:
                    self._obstacle_map[env]._disabled_frontiers.add(tuple(best_frontier))
                    print(f"Frontier {best_frontier} is disabled due to no movement.")
                    # self._obstacle_map[env]._best_frontier_selection_count[frontier_tuple] = 0
        self._last_value[env] = best_value
        ...
        if self._obstacle_map[env]._finish_first_explore == False:
    ...
    def _update_object_map_with_stair_and_person(self, height: int, width: int): # -> Tuple[ ObjectDetections, List[np.ndarray] ]:
        ...
        for env in range(self._num_envs):
            ...
            target_detections, coco_detections, non_coco_detections = self._get_object_detections_with_stair_and_person(rgb, env) # get three detections #target_detections, person_detections, stair_detections 
            ...
            for idx in range(len(target_detections.logits)):
                ...
                self._object_masks[env][target_object_mask > 0] = 1 # for drawing
            ...
            if target_in_coco:
                ...
                non_coco_detections.filter_by_conf(self._non_coco_threshold)
    ...
    def get_floor_probabilities(self, df, target_object_category, floor_num):
        ...
        if floor_num > max_possible_floor:
            # 只处理表格中存在的楼层
            ...
            # 正常处理请求的楼层数
            for y in range(1, floor_num + 1):
                col_name = f"train_floor{floor_num}_{y}"
                probabilities[y] = df.set_index("category").at[target_object_category, col_name] if col_name in df else 0.0
        
        # 补充缺失的楼层（当请求楼层数大于表格最大时不需要补充）
        # if floor_num > max_possible_floor:
        #     for y in range(max_possible_floor + 1, floor_num + 1):
        #         probabilities[y] = 0.0
