...
class HabitatITMPolicy_Qwen_Gdino_DFine_Abla_No_Double_Check_v2(HabitatMixin, ITMPolicyV2):
    def __init__(
        self,
        *args: Any,
        **kwargs: Any,
        ...
        self._object_detector = GroundingDINOClient(port=int(os.environ.get("GROUNDING_DINO_PORT", "12184")))
        self._coco_object_detector = DFineClient(port=int(os.environ.get("DFINE_PORT", "12186")))
        self._mobile_sam = MobileSAMClient(port=int(os.environ.get("SAM_PORT", "12183")))
        self._ram = RAMClient(port=int(os.environ.get("RAM_PORT", "12185")))
        ...
        self._visualize = False # kwargs["visualize"]
        self._vqa_prompt = kwargs["vqa_prompt"]

        self._coco_threshold = kwargs["coco_threshold"] # 0.8 in base_objectnav_policy，感觉0.7也不错？ 
        ...
        self._llm = Qwen2_5ITMClient(port=int(os.environ.get("QWEN2_5ITM_PORT", "12181")))
    ...
    def _explore(self, observations: Union[Dict[str, Tensor], "TensorDict"], env: int, masks: Tensor) -> Tensor:
        ...
        if np.array_equal(frontiers, np.zeros((1, 2))) or len(frontiers) == 0: # no frontier in this floor, check if there is stair
            ...
            # 如果没有未探索的楼梯，尝试在当前楼层探索
            elif not self._obstacle_map[env]._tight_search_thresh:
