...
import torch
import itertools
import falcon.utils.utils_fmm.rotation_utils as ru
...
def transform_camera_view_t_multiple(XYZ, sensor_height, camera_elevation_degrees, device):
    """
    Transforms the point cloud into geocentric frame to account for
    camera elevation and angle
    Input:
    ...
    Rs = []
    for camera_elevation_degree in camera_elevation_degrees:
        R = ru.get_r_matrix([1., 0., 0.], angle=np.deg2rad(camera_elevation_degree))
        R = torch.from_numpy(R).float().transpose(1,0).to(device)
        Rs.append(R)
    XYZs = []
    num_procs = XYZ.shape[0]
    num_points = XYZ.shape[-3]*XYZ.shape[-2]
    for i in range(num_procs):
        XYZs.append(XYZ[i:i+1, :, :, :].reshape(-1,3))
    blockXYZ = torch.block_diag(*XYZs)
    blockR = torch.block_diag(*Rs)
    block_result = torch.matmul(blockXYZ.to(torch.float32), blockR.to(torch.float32))
    
    to_concat = []
    for i in range(num_procs):
        to_concat.append(block_result[num_points*i:num_points*(i+1), 3*i:3*(i+1)])
    before_reshape = torch.cat(to_concat, axis=0) #(22500*num_procs,3)
    XYZ = before_reshape.reshape(XYZ.shape)
...
def splat_feat_nd(init_grid, feat, coords):
    ...
    ones_wt = torch.ones_like(feat[:, [0], :])
    ...
    for d in range(n_dims):
        ...
        for ix in [0, 1]:
            ...
            #wts_ix = torch.round(1 - torch.abs(pos - pos_ix))
