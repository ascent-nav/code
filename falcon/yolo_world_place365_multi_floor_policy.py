...
from constants import MPCAT40_RGB_COLORS, MPCAT40_NAMES, PLACE365_RGB_COLORS
...
# scene map
from vlfm.mapping.scene_map import SceneMap
...
class HabitatITMPolicy_Place365_World(HabitatMixin, ITMPolicyV2):
    def __init__(
        self,
        *args: Any,
        **kwargs: Any,
        ...
        self._object_detector = YoloWorldClient_MF(port=int(os.environ.get("YOLO_WORLD_PORT", "13185")))
        ...
        self._scene_map = [self._scene_map_list[env][self._cur_floor_index[env]] for env in range(self._num_envs)]
    ...
    def _get_policy_info(self, detections: ObjectDetections,  env: int = 0) -> Dict[str, Any]: # seg_map_color:np.ndarray,
        ...
        # policy_info["object_map"] = cv2.cvtColor(self._object_map[env].visualize(self._obstacle_map[env]._map, 
        ...
        #                                         ), cv2.COLOR_BGR2RGB) # self._observations_cache[env]["habitat_start_yaw"]
        policy_info["scene_map"] = cv2.cvtColor(self._scene_map[env].visualize(PLACE365_RGB_COLORS), cv2.COLOR_BGR2RGB)
    ...
    def _update_scene_map(self) -> None:
        for env in range(self._num_envs):
            self._scene_map[env].update_map(
                    self._observations_cache[env]["object_map_rgbd"][0][1],
                    self._observations_cache[env]["object_map_rgbd"][0][2],
                    self._min_depth,
                    self._max_depth,
                    ...
                    self._fy,
                    self._camera_fov,
                    self._obstacle_map[env]._navigable_map,
                    self.scene_classify_list[env],
                )
    ...
    def act(
        ...
        for env in range(self._num_envs):
            ...
            rgb_image = rgb.squeeze(0).cpu().numpy().astype(np.uint8)
            ...
            self.scene_classify_list.append(idx[0].cpu().item())
            ...
            if DEBUG:
                import matplotlib.pyplot as plt
                import matplotlib.patches as mpatches
                debug_dir = "debug/20250203/seg_scene" # seg_debug_up_climb_stair
                os.makedirs(debug_dir, exist_ok=True)  # 确保调试目录存在
                if not hasattr(self, "step_count"):
                    self.step_count = 0  # 添加 step_count 属性
                # 将 step 计数器用于文件名
                ...
                plt.savefig(filename, bbox_inches="tight", pad_inches=0, format="png", pil_kwargs={"mode": "RGB"})
                plt.close()  # 关闭图像窗口
                print(f"Saved debug RGB image to {filename}")
    ...
    def _get_object_detections_with_stair_and_person(self, img: np.ndarray, env: int) -> ObjectDetections:
        ...
        # "tv_monitor", # "picture"
        base_caption = [["chair"], ["couch"], ["potted plant"], ["bed"], ["toilet"], ["tv"], ["stair"], ["person"], [" "]]        # 去掉 all_caption 中已存在的元素
