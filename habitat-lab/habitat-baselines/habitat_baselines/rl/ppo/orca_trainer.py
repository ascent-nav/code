...
@baseline_registry.register_trainer(name="orca-notrainer")
class ORCANoTrainer(BaseRLTrainer):
    r"""Trainer class for PPO algorithm
    Paper: https://arxiv.org/abs/1707.06347.
    """
    supported_tasks = ["Nav-v0"] # socialnav-V2's datasets actually belongs to Nav-v0, for easy to use
    ...
    def _eval_checkpoint(
        ...
        if len(self.config.habitat_baselines.eval.video_option) > 0:
            # only need to update the sensors of main agent (agent_0)
        ...
        # self._agent._agents[0].policy.set_sim(self.sim, self._agent._agents[0])
