...
class _ObservationBatchingCache(metaclass=Singleton):
    ...
    def batch_obs(
        ...
        # because original vlfm may have multiple frontiers, but unsuitable for multi-env setup
        observation_tensors = []
        for obs in observations:
            if 'frontier_sensor' in obs:
                if obs['frontier_sensor'].shape != (1, 2):
                    obs['frontier_sensor'] = obs['frontier_sensor'][0:1] 
            observation_tensors.append(obs.flatten()[1])
