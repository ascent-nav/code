...
class LocalizationSensor(UsesArticulatedAgentInterface, Sensor):
    ...
    def get_observation(self, observations, episode, *args, **kwargs):
        if isinstance(self, ArticulatedAgentData):
            ...
            rotation = self._sim.get_agent_state(self.agent_id).rotation
            # 提取四元数的分量
            agent_yaw = - 2 * np.arctan2(rotation.y, rotation.w)
            # 将 agent_yaw 归一化到 [-π, π]
            agent_yaw = (agent_yaw + np.pi) % (2 * np.pi) - np.pi
            # rad_rotation = quaternion_to_rad_angle(np.array([rotation.w, rotation.x, rotation.y, rotation.z], dtype=np.float32))
            return np.append(self._sim.get_agent_state(self.agent_id).position, agent_yaw)
