...
@registry.register_simulator(name="RearrangeSim-v2")
class RearrangeSim_v2(HabitatSim):
    ...
    def set_agent_state(
        ...
        agent.set_state(new_state, reset_sensors,is_initial=True)
        # self.internal_step(-1, update_articulated_agent=False)
        return True
    
    # my added
    def step_filter(self, start_pos: mn.Vector3, end_pos: mn.Vector3, agent_index: int = None) -> mn.Vector3:
        r"""Computes a valid navigable end point given a target translation on the NavMesh.
        Uses the configured sliding flag.
        Different from the default, continuously use allow sliding for humans (except agent_0).
        ...
        if self.pathfinder.is_loaded:
            if self.config.sim_cfg.allow_sliding or agent_index is not None:
    ...
    def reconfigure(self, config: "DictConfig", ep_info: RearrangeEpisode):
        ...
        if "object_labels" in ep_info.info: # my added, to differ from the social navigation task and social rearrange task
            ...
            self.agents_mgr.pre_obj_clear() # agent clear
            # self._clear_objects(should_add_objects, new_scene)
            ...
            if is_hard_reset:
                with read_write(config):
                    config["scene"] = ep_info.scene_id
                t_start = time.time()
                super().reconfigure(config, should_close_on_new_scene=True) # modify to True
                self.add_perf_timing("super_reconfigure", t_start)
                # The articulated object handles have changed.
                self._start_art_states = {}
            ...
            # ao_state is {} even in hssd
            if hasattr(ep_info, "ao_states"):
                pass
            else:
                ep_info.ao_states = {}
            # Load specified articulated object states from episode config
            ...
            # add episode clutter objects additional to base scene objects
            # if self._load_objs: # episode has no objects in fact
            #     self._add_objs(ep_info, should_add_objects, new_scene)
            # self._setup_targets(ep_info)
    ...
    def num_articulated_agents(self):
        num = len(self.agents_mgr)
        for agent_data in self.agents_mgr._all_agent_data:
            if isinstance(agent_data, ArticulatedAgentData):
                pass
            else:
                num -= 1
        return num
    ...
    def _load_navmesh(self, ep_info):
        ...
        # the path is for human # navmeshes_human
    ...
    def _create_obj_viz(self):
        ...
        if self._debug_render_goal and hasattr(self, "_targets"):
    ...
    def get_agent_state(self, agent_id: int = 0) -> habitat_sim.AgentState:
        ...
        if isinstance(agent_data, ArticulatedAgentData):
            ...
            return self.agents[0].state # make sure that object of agent can be specified # self.get_agent(agent_id).get_state()
    ...
    def step(self, action: Union[str, int]) -> Observations:
        ...
        if self._batch_render:
            ...
            if isinstance(self.agents_mgr._sim.agents_mgr._all_agent_data[0], ArticulatedAgentData):
                obs = self._sensor_suite.get_observations(
                    self.get_sensor_observations()
                )
            else: 
                obs = super().step(action)    # seems it is the final obs so           
