...
from habitat.tasks.rearrange.rearrange_sim_v2 import (
    RearrangeSim_v2,
)
...
@registry.register_task(name="MultiAgentObjectNavTask-v0")
class MultiAgentObjectNavTask(NavigationTask):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # if hasattr(self._sim, "agents_mgr") and len(self._sim.agents_mgr) > 1: # add one agent situation
    ...
    def overwrite_sim_config(self, config: Any, episode: Episode) -> Any:
        # refer to navigation task, 不能这样写，否则会给每个人重复赋予坐标，多余。
        # return super().overwrite_sim_config(config, episode) 
        return config
    ...
    def _set_articulated_agent_start(self, agent_idx: int) -> None:
        ...
        if self._use_episode_start_goal:
            ...
            if self._min_distance_start_agents > 2.0:
        ...
        ## safe_snap_point for humanoid agents
        # for attempt_i in range(100): # max_attempts = 100
        #     start_pos = self._sim.safe_snap_point(articulated_agent_pos)
    ...
    def reset(self, episode: Episode, fetch_observations: bool = True):
        ...
        if self._sim_reset:
            # 原先的这一步，在HabitatSim 完成，这里避免重复赋值。
            if self._should_place_articulated_agent:
                for agent_idx, agent in enumerate(self._sim.agents_mgr._all_agent_data):
                    if isinstance(agent, ArticulatedAgentData): 
                        self._set_articulated_agent_start(agent_idx)
                    else:
                        self._sim.set_agent_state(
                            [float(k) for k in episode.start_position],
    ...
    def _check_episode_is_active(
        ...
        **kwargs: Any,
        ...
        if self.should_end or self.is_stop_called:
            done = True
