...
TOP_DOWN_MAP_COLORS[MAP_HUMAN_3_TRAJECTORY] = [0,255,0] # green
...
def draw_agent(
    ...
    # imageio.imwrite(f'video_topdown_0308/topdown_draw_agent{agent_idx}.png', image)
    return image
...
def get_topdown_map(
    ...
    # # 使用imshow绘制地图，并应用之前定义的颜色映射
    # plt.figure(figsize=(10, 6))
    # plt.imshow(top_down_map, cmap=cmap, interpolation='nearest')
    # plt.colorbar(ticks=[0, 1, 2])
    # plt.title('Top-Down Map Visualization')
    # plt.savefig('video_topdown_0308/occupied_map') it is normal
...
def get_topdown_map_from_sim(
    ...
    """
    if hasattr(sim,"agents_mgr") and isinstance(sim.agents_mgr._all_agent_data[agent_id], ArticulatedAgentData): # hasattr(sim, "num_articulated_agents") and sim.num_articulated_agents > 1:
        return get_topdown_map(
            sim.pathfinder,
            sim.get_agent_data(agent_id).articulated_agent.base_pos[1],
            # sim.agents_mgr._all_agent_data[agent_id].cfg.position.y,
            map_resolution,
            draw_border,
            meters_per_pixel,
        )
        ...
        return get_topdown_map(
            sim.pathfinder,
            # sim.get_agent(agent_id).state.position[1], # 这里有问题，多代理情况下应该是agent_0（而不是get_agent得到的）
            sim.get_agent_state(agent_id).position[1],
            map_resolution,
            draw_border,
            meters_per_pixel,
        )
...
def colorize_topdown_map(
    ...
    # imageio.imwrite('video_topdown_0308/_map.png', _map)
    ...
    # imageio.imwrite('video_topdown_0308/_map_after_fog.png', _map)
...
def colorize_draw_agent_and_fit_to_height(
    ...
    # imageio.imwrite('video_topdown_0308/ini_topdownmap.png', top_down_map)
    top_down_map = colorize_topdown_map(
        top_down_map, topdown_map_info["fog_of_war_mask"]
    )
    # imageio.imwrite('video_topdown_0308/topdownmap.png', top_down_map)
    ...
    # imageio.imwrite('video_topdown_0308/topdown_example2.png', top_down_map)
    return top_down_map
