...
def observations_to_image(observation: Dict, info: Dict) -> np.ndarray:
    ...
    multi_floor_top_down_map_key = "multi_floor_map"
    ...
    elif multi_floor_top_down_map_key in info:
        multi_floor_map = maps.colorize_draw_agent_and_fit_to_height(
            info[multi_floor_top_down_map_key], render_frame.shape[0]
        )
        render_frame = np.concatenate((render_frame, multi_floor_map), axis=1)
...
def overlay_frame(frame, info, additional=None):
    ...
    elif "multi_floor_map" in info:
        info.pop("multi_floor_map")
