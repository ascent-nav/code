...
for scene_count, json_str in enumerate(os.listdir(folder_path)):
    ...
    while add_human_amount >= 1:
        success_episode = 0  # Reset success_episode count for this add_human_num attempt
        ...
        # After processing all episodes in the scene, check success_episode count
        if success_episode >= (episode_total_num // 2):
            break  # Enough successful episodes, proceed to next scene
        else:
            # Reduce add_human_num and try again
            add_human_amount -= 1
            print(f"Retrying scene {scene_id} with {add_human_amount} humans due to insufficient success episodes.")

    # If add_human_num reaches 0 and we still have less than 10 successful episodes
    if success_episode < (episode_total_num // 2):
        print(f"Scene {scene_id} filtered out due to insufficient success episodes.")
        # continue

    # Store episode data
    scene_episode_data.append({
        "dataset": "hm3d_pointnav_add_human", 
        "split": split,
        "scene": scene_id,
