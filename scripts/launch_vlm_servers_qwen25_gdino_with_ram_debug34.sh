...
tmux send-keys -t ${session_name}:0.3 "export CUDA_VISIBLE_DEVICES=3; ${VLFM_PYTHON} -m vlfm.vlm.grounding_dino_test --port ${GROUNDING_DINO_PORT}" C-m
tmux send-keys -t ${session_name}:0.4 "export CUDA_VISIBLE_DEVICES=3; ${VLFM_PYTHON} -m vlfm.vlm.ram_test --port ${RAM_PORT}" C-m
tmux send-keys -t ${session_name}:0.5 "export CUDA_VISIBLE_DEVICES=3; ${VLFM_PYTHON} -m vlfm.vlm.yolov7 --port ${YOLOV7_PORT}" C-m
