...
tmux send-keys -t ${session_name}:0.1 "export CUDA_VISIBLE_DEVICES=2; ${VLFM_PYTHON} -m vlfm.vlm.blip2itm --port ${BLIP2ITM_PORT}" C-m
tmux send-keys -t ${session_name}:0.2 "export CUDA_VISIBLE_DEVICES=1; ${VLFM_PYTHON} -m vlfm.vlm.sam --port ${SAM_PORT}" C-m
tmux send-keys -t ${session_name}:0.3 "export CUDA_VISIBLE_DEVICES=1; ${VLFM_PYTHON} -m vlfm.vlm.yolo_world_test --port ${YOLO_WORLD_PORT}" C-m
tmux send-keys -t ${session_name}:0.4 "export CUDA_VISIBLE_DEVICES=2; ${VLFM_PYTHON} -m vlfm.vlm.ram_test --port ${RAM_PORT}" C-m
