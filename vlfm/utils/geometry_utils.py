...
def inverse_transform_points(transformation_matrix: np.ndarray, points_3d: np.ndarray) -> np.ndarray:
    """
    ...
    """
    if points_3d.ndim == 1:
        points_3d = np.expand_dims(points_3d, axis=0) 
    # 将点转换为齐次坐标 (N, 4)
    ...
    # 应用逆变换矩阵
    transformed_points = np.dot(inverse_matrix, homogeneous_points.T).T
...
def project_points_to_image(points_3d: np.ndarray, fx: float, fy: float, width: int, height: int) -> np.ndarray:
    """
