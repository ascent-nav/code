    ...
    def _eval_checkpoint(
        ...
        while len(stats_episodes) < (number_of_eval_episodes * evals_per_ep) and self.envs.num_envs > 0:
            ...
            with inference_mode():
                ...
                # else:
                #     for i, should_insert in enumerate(action_data.should_inserts[0]):
            ...
            for i in range(n_envs):
                ...
                if len(self.config.habitat_baselines.eval.video_option) > 0:
                    # hab_vis.collect_data_with_third_view(batch, infos, action_data.policy_info, i)
                    # hab_vis.collect_data_with_third_view_and_seg_map(batch, infos, action_data.policy_info, i)
                    # hab_vis.collect_data_with_third_view_seg_map_object_map(batch, infos, action_data.policy_info, i)
                    # collect_data_with_third_view_and_seg_map_vlm_input
                    if "vlm_response" in action_data.policy_info[i]:       
                        ...
                        hab_vis.collect_data_with_third_view_and_seg_map(batch, infos, action_data.policy_info, i)
                    # if "num_steps" in action_data.policy_info[i] and action_data.policy_info[i]["num_steps"] == 1:
                ...
                if not not_done_masks[i].item():
                    ...
                    print(f"Average Spl: {num_spl / num_total * 100:.2f}%")
                    print(f"Average Dtg: {num_dtg / num_total}")
                    from vlfm.utils.episode_stats_logger import (
                        log_episode_stats,
                    )
                    import vlfm.utils.failure_logger as failure_logger
                    ...
                    if len(self.config.habitat_baselines.eval.video_option) > 0:
                        if "vlm_response" in action_data.policy_info[i]:
                            rgb_frames[i] = hab_vis.flush_frames_with_rednet_vlm_input(failure_cause, i) # flush_frames
                        else:
                            rgb_frames[i] = hab_vis.flush_frames_with_rednet(failure_cause, i)
                        # rgb_frames[i] = hab_vis.flush_frames_with_rednet_and_object_map(failure_cause, i)
                        # rgb_frames[i] = hab_vis.flush_frames_with_rednet(failure_cause, i)
                        generate_video(
