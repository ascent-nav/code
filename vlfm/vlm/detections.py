...
class ObjectDetections:
    ...
    def filter_by_class_id(self, classes: List[str]) -> None:
        """Filters detections by class in-place.
    ...
    def get_bboxes_for_class(self, class_name: str, fill_depth: np.ndarray) -> np.ndarray:
        """
        获取指定类别的边界框。

        Args:
            class_name (str): 目标类别的名称，例如 "person"。
            fill_depth (Union[np.ndarray, Any])) : 深度图，形如(480,640)
        Returns:
            List[Tuple[int, int, int, int]]: 包含所有检测到的目标类别边界框的列表，
            每个框的格式为 (x1, y1, x2, y2)。
        """
        if len(self.phrases) == 0:
            return None
        person_mask = np.zeros_like(fill_depth, dtype=bool)
        height, width = fill_depth.shape[:2] 
        for i, phrase in enumerate(self.phrases):
            if phrase == class_name:
                bbox_denorm = (self.boxes[i] * np.array([width, height, width, height])).int().cpu().numpy() # 假设 box 是 [x1, y1, x2, y2] 格式
    ...
    def filter_by_class_and_conf(self, class_conf_thresholds: dict) -> None:
        """Filters detections by class and confidence threshold in-place.

        Args:
            class_conf_thresholds (dict): Dictionary of class names to confidence thresholds.
        """
        keep = torch.zeros_like(self.logits, dtype=torch.bool)
        for i, (phrase, logit) in enumerate(zip(self.phrases, self.logits)):
            conf_threshold = class_conf_thresholds.get(phrase, 0.0)  # 默认阈值为0.0
